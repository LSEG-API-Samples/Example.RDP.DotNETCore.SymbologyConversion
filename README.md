...
Basically, the output from the NSwageStudio is a C# client class with a Data Models and its Enum values. The client classes basically provide an interface for application to request and manage the response. The client class uses a NewtonSoft JSON library to handle the JSON data. It can use the JSON library to serialize the object to JSON request and deserialize the response back to the Symbology object. The NSwagStudio automatic generate .NET async method for retrieving the data using Http Get or Http Post. Please refers to the instruction for using the NSWagStudio application from [NSWagStudio github page](https://github.com/RSuter/NSwag/wiki/NSwagStudio). Note that after generating the client class form JSON scheme, you might need to refactor the codes and modify the class name and enum value yourself to match with your application design.


#### Interface and Class for EDS Authentication

The example also use SWagger JSON scheme to generate client class for EDS Authentication.
...
**Tokenresponse** is a class used when the application deserialize the response body. All class members represent the Token data.
...
User may also use --verbose to print additional informaiton such as the JSON request and response.
...
As described earlier, user has to enter EDP username and password to get AccessToken and then pass the token to Symbology Convert request to get the data object.
Below sample is the output when you run the command line. It will three set of array inside the data object.Each of element inside the array is the value of the fields the applicaiton request from the server. In this case, it returns all available fields because user does not specify fields list.
